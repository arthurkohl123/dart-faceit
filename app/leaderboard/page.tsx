...
export default function Leaderboard() {
  ...
  const podiumConfig = [
    ...
    { 
      ...
      prize: '3M Premium', 
      height: 'h-[240px]', 
      color: 'bg-yellow-400/15 border-yellow-400/40 shadow-[0_0_50px_rgba(250,204,21,0.1)]', 
      label: 'CHAMPION', 
      isWinner: true 
    },
    { 
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-black/90 backdrop-blur-2xl border-b border-white/5 py-3' : 'bg-transparent py-8'}`}>
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-12 text-[11px] font-black uppercase tracking-[0.3em] text-zinc-400">
            ...
            <Link href="/profile" className="hover:text-white transition-all flex items-center gap-2">
          ...
          <div className="flex items-center gap-6">
            <button 
              ...
              className="hidden sm:block px-8 py-3 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/10 text-[10px] font-black uppercase tracking-widest transition-all"
            >
              {isLoggedIn ? 'Dashboard' : 'Sign In'}
            </button>
            <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="lg:hidden p-2 text-zinc-400">
          ...
          <div className="flex flex-col md:flex-row items-end justify-between mb-24 gap-8">
            <div className="space-y-6 text-center md:text-left">
              ...
              <p className="text-zinc-500 text-lg max-w-xl font-medium leading-relaxed">
                Dominiere das Feld und sichere dir <span className="text-emerald-400 font-black italic">Gratis Premium-Mitgliedschaften</span>.
              </p>
            </div>
