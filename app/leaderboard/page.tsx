  ...
  return (
    <main className="arena-page text-[#f5f3ee]">
      ...
      <nav className="arena-nav border-b border-white/10 bg-[#0a0d0d]/95 backdrop-blur-md">
        ...
        {mobileMenuOpen && <div className="border-t border-white/10 px-5 py-3 lg:hidden"><div className="grid gap-1 text-sm font-bold text-zinc-300">{[['Matchmaking', '/matchmaking'], ['Spielzeiten', '/playtimes'], ['Mittwoch Showdown', '/showdown'], ['Turniere', '/tournaments'], ['History', '/history'], ['Premium', '/premium']].map(([label, href]) => <Link key={href} href={href} onClick={() => setMobileMenuOpen(false)} className="border-b border-white/5 py-3 hover:text-emerald-200">{label}</Link>)}</div></div>}
      </nav>
