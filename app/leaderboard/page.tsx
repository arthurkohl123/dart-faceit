  ...
  return (
    <main className="min-h-screen bg-[#0a0d0d] text-[#f5f3ee]">
      ...
      <nav className="border-b border-white/10 bg-[#0a0d0d]/95 backdrop-blur-md">
        ...
        {mobileMenuOpen && <div className="border-t border-white/10 px-5 py-3 lg:hidden"><div className="grid gap-1 text-sm font-bold text-zinc-300">{[['Matchmaking', '/matchmaking'], ['Mittwoch Showdown', '/showdown'], ['Turniere', '/tournaments'], ['Mein Profil', '/profile'], ['Updates', '/updates'], ['Premium', '/premium']].map(([label, href]) => <Link key={href} href={href} onClick={() => setMobileMenuOpen(false)} className="border-b border-white/5 py-3 hover:text-emerald-200">{label}</Link>)}</div></div>}
      </nav>
