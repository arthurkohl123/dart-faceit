  ...
  return (
    <main className="min-h-screen bg-[#0a0d0d] text-[#f5f3ee]">
      ...
      <section className="mx-auto max-w-7xl px-5 py-12 md:px-8 md:py-16">
        ...
        {topPlayers.length === 3 && <section className="mt-10"><div className="mb-4 flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em] text-zinc-500"><Crown className="h-4 w-4 text-amber-200" /> Spitze der Rangliste</div><div className="grid border-t border-white/15 md:grid-cols-3">{topPlayers.map((player, index) => { const rank = getRankForElo(player.elo); const winrate = player.gamesPlayed ? Math.round((player.wins / player.gamesPlayed) * 100) : 0; return <Link key={player.username} href={`/players/${encodeURIComponent(player.username)}`} className={`group border-b border-white/15 px-5 py-7 transition hover:bg-white/[.025] md:border-r md:last:border-r-0 ${index === 0 ? 'md:order-2' : index === 1 ? 'md:order-1' : 'md:order-3'}`}><div className="flex items-center justify-between"><span className="text-2xl">{medals[index]}</span><span className="text-xs font-black uppercase tracking-[.14em] text-zinc-500">Platz {index + 1}</span></div><div className="mt-8"><span className={`text-xl font-black tracking-[-.04em] ${player.isPremium ? premiumNameStyle : ''}`}>{player.username}</span><p className={`mt-1 text-xs font-black ${rank.color}`}>L{rank.level} · {rank.name}</p></div><div className="mt-8 flex items-end justify-between border-t border-white/10 pt-4"><div><p className="text-4xl font-black tracking-[-.06em] text-emerald-300">{player.elo}</p><p className="text-[10px] font-black uppercase tracking-[.13em] text-zinc-500">Elo</p></div><div className="text-right text-xs"><p className="font-black">{winrate}%</p><p className="text-zinc-500">Winrate</p></div></div></Link>; })}</div></section>}

        <section className="mt-12 border border-white/15 bg-[#0d1110]"><div className="flex items-center justify-between border-b border-white/15 px-5 py-5 md:px-6"><div><p className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em] text-emerald-300"><Medal className="h-4 w-4" /> Globale Rangliste</p><p className="mt-1 text-sm text-zinc-500">{searchQuery ? `${filteredPlayers.length} Treffer für „${searchQuery}“` : 'Top 100 · Nur bestätigte Ranked-Matches'}</p></div><Link href="/matchmaking" className="hidden border border-emerald-300/35 px-4 py-2 text-xs font-black text-emerald-200 hover:bg-emerald-300/10 sm:block">Match suchen</Link></div><div className="hidden grid-cols-[4rem_1fr_5rem_5rem_6rem_5rem_5rem] gap-4 border-b border-white/10 px-6 py-3 text-[10px] font-black uppercase tracking-[.13em] text-zinc-500 md:grid"><span>Platz</span><span>Spieler</span><span>Spiele</span><span>Winrate</span><span>Ø Average</span><span>Elo</span><span>Preis</span></div><div className="divide-y divide-white/10">{filteredPlayers.map(({ player, index }) => { const rank = getRankForElo(player.elo); const winrate = player.gamesPlayed ? Math.round((player.wins / player.gamesPlayed) * 100) : 0; const prize = [175, 100, 75, 50, 30][index]; return <Link key={`${player.username}-${index}`} href={`/players/${encodeURIComponent(player.username)}`} className="grid grid-cols-[2.8rem_1fr_auto] items-center gap-3 px-5 py-4 transition hover:bg-emerald-300/[.045] md:grid-cols-[4rem_1fr_5rem_5rem_6rem_5rem_5rem] md:gap-4 md:px-6"><span className={`grid h-8 w-8 place-items-center text-xs font-black ${index < 3 ? 'bg-amber-300 text-black' : 'border border-white/10 text-zinc-400'}`}>{index < 3 ? medals[index] : `#${index + 1}`}</span><span className="min-w-0"><span className={`block truncate text-sm font-black ${player.isPremium ? premiumNameStyle : ''}`}>{player.username}</span><span className={`mt-1 block text-xs font-bold ${rank.color}`}>L{rank.level} · {rank.name}</span></span><span className="hidden text-sm font-black md:block">{player.gamesPlayed}</span><span className="hidden text-sm font-black text-zinc-200 md:block">{winrate}%</span><span className="hidden text-sm font-black text-zinc-200 md:block">{player.supabaseId && avgMap[player.supabaseId] != null ? avgMap[player.supabaseId].toFixed(1) : '—'}</span><span className="text-right"><span className="block text-lg font-black text-emerald-300">{player.elo}</span><span className="text-[10px] font-bold text-zinc-500 md:hidden">Elo</span></span><span className="hidden text-xs font-black text-amber-100 md:block">{prize ? `${prize} €` : '—'}</span></Link>; })}</div>{filteredPlayers.length === 0 && <div className="px-6 py-16 text-center"><Search className="mx-auto h-7 w-7 text-zinc-600" /><p className="mt-3 font-black">Kein Spieler gefunden</p><p className="mt-1 text-sm text-zinc-500">Versuche es mit einem anderen Namen.</p></div>}</section>
