...
/**
 * PROFESSIONELLE RANK ICONS - Original Design
 */
  ...
  const fetchLeaderboard = useCallback(async () => {
    try {
      ...
      if (!error && data) {
        ...
        if (ids.length > 0) {
          // Schnelle Batch-Abfrage für Durchschnittswerte
                ...
                return (
                  <div key={item.index} className={`relative flex flex-col items-center w-full md:w-1/3 ${item.index === 0 ? 'order-1 md:order-2' : item.index === 1 ? 'order-2 md:order-1' : 'order-3'}`}>
                    ...
                    <div className={`relative w-full ${item.height} ${item.color} border-t-2 rounded-t-[3rem] flex flex-col items-center justify-start pt-10 overflow-hidden group/pillar transition-all duration-700 hover:bg-white/[0.08]`}>
                      ...
                      <div className="relative px-8 py-4 rounded-[2rem] bg-black/40 border border-white/10 text-[10px] font-black uppercase tracking-widest text-emerald-400 shadow-2xl">{item.prize}</div>
                    </div>
                  </div>
                );
              })}
          ...
          <div className="bg-zinc-900/20 border border-white/5 rounded-[3rem] overflow-hidden backdrop-blur-md">
            <div className="p-10 border-b border-white/5 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-emerald-500/10 rounded-2xl flex items-center justify-center text-emerald-500 shadow-[0_0_30px_rgba(16,185,129,0.1)]"><Activity className="w-6 h-6" /></div>
                <div>
                ...
                return (
                  <Link href={`/players/${encodeURIComponent(player.username)}`} key={i} className="group flex items-center justify-between p-8 hover:bg-white/[0.02] transition-all">
                    ...
                    <div className="hidden md:flex items-center justify-between w-2/3">
                      ...
                      <div className="text-center w-24"><div className="text-lg font-black italic text-zinc-200">{Math.round((player.wins / player.gamesPlayed) * 100)}%</div><div className="text-[8px] font-black uppercase tracking-widest text-zinc-600">Win Rate</div></div>
