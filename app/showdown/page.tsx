...
const prizes = ['15 €', '14 Tage Premium', '7 Tage Premium'];
const medals = ['🥇', '🥈', '🥉'];
    ...
    <main className="min-h-screen bg-[#0a0d0d] text-[#f5f3ee]">
      ...
      <nav className="border-b border-white/10 bg-[#0a0d0d]/95 backdrop-blur-md">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-5 py-4 md:px-8">
          <Link href="/" className="flex items-center gap-3"><BrandLogo className="h-10 w-10 rounded-lg" /><span><span className="block text-lg font-black tracking-[-.05em]">RANKEDDARTS</span><span className="block text-[9px] font-bold uppercase tracking-[.25em] text-violet-300">Mittwoch Showdown</span></span></Link>
      ...
      <section className="mx-auto max-w-7xl px-5 py-12 md:px-8 md:py-16">
        <header className="relative overflow-hidden border border-violet-300/20 bg-[linear-gradient(135deg,rgba(109,40,217,0.18),rgba(13,17,16,0.96)_54%,rgba(10,13,13,0.98))] p-6 md:p-10">
          ...
          <div className="relative grid gap-8 lg:grid-cols-[1fr_19rem] lg:items-end">
            <div>
              ...
              <h1 className="mt-5 text-5xl font-black leading-[.88] tracking-[-.075em] md:text-7xl">Mittwoch<br /><span className="text-violet-300">Showdown.</span></h1>
              <p className="mt-5 max-w-2xl text-base leading-7 text-zinc-300">Vier Stunden, eine eigene Wochenwertung. Spiele ganz normal Ranked – deine Elo zählt weiter für die Saison und gleichzeitig für den Showdown.</p>
            </div>
        ...
        <div className="mt-5 grid border border-white/10 bg-[#0d1110] md:grid-cols-4">
          <div className="border-b border-r border-white/10 px-5 py-4 md:border-b-0"><Clock3 className="h-4 w-4 text-violet-300" /><p className="mt-2 text-sm font-black">18:00–22:00 Uhr</p><p className="mt-1 text-[10px] font-black uppercase tracking-[.13em] text-zinc-500">Jeden Mittwoch</p></div>
          ...
          <div className="px-5 py-4"><Trophy className="h-4 w-4 text-amber-200" /><p className="mt-2 text-sm font-black">15 € + Premium</p><p className="mt-1 text-[10px] font-black uppercase tracking-[.13em] text-zinc-500">Top 3 Preise</p></div>
        </div>

        <section className="mt-12 overflow-x-auto border border-white/15 bg-[#0d1110]">
          <div className="flex min-w-[780px] items-center justify-between border-b border-white/15 px-6 py-5"><div><p className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em] text-violet-300"><Medal className="h-4 w-4" /> Mittwoch Showdown · Rangliste</p><p className="mt-1 text-sm text-zinc-500">{active ? `Live · mindestens ${qualifierText} · Sortierung nach Siegen, Winrate und Average` : 'Die Wertung ist ausschließlich mittwochs zwischen 18:00 und 22:00 Uhr sichtbar.'}</p></div><span className={`border px-3 py-1 text-[10px] font-black uppercase tracking-[.14em] ${active ? 'border-emerald-300/30 text-emerald-200' : 'border-white/10 text-zinc-500'}`}>{active ? `${players.length} qualifiziert` : 'Inaktiv'}</span></div>
