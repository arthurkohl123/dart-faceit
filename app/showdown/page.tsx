...
const defaultConfig: ShowdownConfig = {
  description: 'Vier Stunden, eine eigene Wochenwertung. Bestätigte Ranked-Matches aus der Queue und aus Turnieren zählen mit – deine Elo läuft ganz normal weiter.',
  prize_first: '15 €',
...
export default function ShowdownPage() {
  ...
  // A repeat can already have a valid standing before its four-hour window
  // opens: yesterday's interrupted window is deliberately carried forward.
  const showLeaderboard = active || players.length > 0;
  ...
  return (
    <main className="min-h-screen bg-[#0a0d0d] text-[#f5f3ee]">
      ...
      <nav className="border-b border-white/10 bg-[#0a0d0d]/95 backdrop-blur-md">
        ...
        {mobileMenuOpen && <div className="border-t border-white/10 px-5 py-3 lg:hidden"><div className="grid gap-1 text-sm font-bold text-zinc-300">{[['Matchmaking', '/matchmaking'], ['Rangliste', '/leaderboard'], ['Turniere', '/tournaments'], ['Premium', '/premium']].map(([label, href]) => <Link key={href} href={href} onClick={() => setMobileMenuOpen(false)} className="border-b border-white/5 py-3 hover:text-violet-200">{label}</Link>)}</div></div>}
      </nav>
      ...
      <section className="mx-auto max-w-7xl px-5 py-12 md:px-8 md:py-16">
        <header className="relative overflow-hidden border border-violet-300/20 bg-[linear-gradient(135deg,rgba(109,40,217,0.18),rgba(13,17,16,0.96)_54%,rgba(10,13,13,0.98))] p-6 md:p-10">
          ...
          <div className="relative grid gap-8 lg:grid-cols-[1fr_19rem] lg:items-end">
            <div>
              <div className={`inline-flex items-center gap-2 border px-3 py-1 text-[10px] font-black uppercase tracking-[.18em] ${active ? 'border-emerald-300/30 bg-emerald-400/10 text-emerald-100' : 'border-violet-300/25 bg-violet-400/10 text-violet-100'}`}><span className={`h-2 w-2 rounded-full ${active ? 'animate-pulse bg-emerald-300' : 'bg-violet-300'}`} />{active ? `Live · ${eventHours}` : isMakeupRepeat ? `Heute · Wiederholung · ${eventHours}` : eventAvailable ? 'Wöchentlich · Mittwoch' : 'Derzeit pausiert'}</div>
              <p className="mt-7 border-l-2 border-violet-300 pl-3 text-[11px] font-black uppercase tracking-[.2em] text-violet-200">Queue & Turniere · normale Elo-Wertung bleibt aktiv</p>
            ...
            <div className="border border-white/15 bg-black/25 p-5">
              <p className="text-[10px] font-black uppercase tracking-[.16em] text-zinc-500">{active ? 'Showdown läuft bis' : isMakeupRepeat ? 'Wiederholung heute' : 'Nächster Showdown'}</p>
        ...
        <div className="mt-5 grid border border-white/10 bg-[#0d1110] md:grid-cols-4">
          <div className="border-b border-r border-white/10 px-5 py-4 md:border-b-0"><Clock3 className="h-4 w-4 text-violet-300" /><p className="mt-2 text-sm font-black">{eventHours}</p><p className="mt-1 text-[10px] font-black uppercase tracking-[.13em] text-zinc-500">{isMakeupRepeat ? 'Einmalig heute' : 'Jeden Mittwoch'}</p></div>
        ...
        <section className="mt-12 overflow-x-auto border border-white/15 bg-[#0d1110]">
          <div className="flex min-w-[780px] items-center justify-between border-b border-white/15 px-6 py-5"><div><p className="flex items-center gap-2 text-[11px] font-black uppercase tracking-[.18em] text-violet-300"><Medal className="h-4 w-4" /> {status?.title ?? 'Mittwoch Showdown'} · Rangliste</p><p className="mt-1 text-sm text-zinc-500">{active ? `Live · mindestens ${qualifierText} · Sortierung nach Siegen, Winrate und Average` : showLeaderboard && isMakeupRepeat ? `Zwischenstand aus gestern · heute zwischen ${eventHours} werden weitere Matches dazugerechnet.` : `Die Wertung ist ausschließlich mittwochs zwischen ${eventHours} sichtbar.`}</p></div><span className={`border px-3 py-1 text-[10px] font-black uppercase tracking-[.14em] ${showLeaderboard ? 'border-emerald-300/30 text-emerald-200' : 'border-white/10 text-zinc-500'}`}>{showLeaderboard ? `${players.length} qualifiziert` : 'Inaktiv'}</span></div>
