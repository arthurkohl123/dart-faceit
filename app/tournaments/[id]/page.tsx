...
function KnockoutBracket({ tournament, matches, userId }: { tournament: Tournament; matches: TournamentMatch[]; userId: string | null }) {
  ...
  return <section className="border border-white/10 bg-[#0d1110]"><header className="flex flex-col justify-between gap-2 border-b border-white/10 p-5 sm:flex-row sm:items-end"><div><p className="text-[10px] font-black tracking-[.16em] text-amber-300">TURNIERPLAN</p><h2 className="mt-1 text-2xl font-black">Der Weg zum Pokal</h2></div>{tournament.winner_username && <span className="w-fit bg-amber-300/10 px-3 py-2 text-sm font-black text-amber-100">Champion: {tournament.winner_username}</span>}</header>{matches.length === 0 ? <EmptyPlan /> : <div className="grid gap-5 p-5 md:grid-cols-2 xl:grid-cols-3">{Object.entries(rounds).sort(([left], [right]) => Number(left) - Number(right)).map(([round, roundMatches]) => <section key={round}><p className="mb-3 text-[10px] font-black tracking-[.15em] text-zinc-400">RUNDE {round}</p><div className="space-y-3">{roundMatches.map(match => <GroupMatchCard key={match.id} match={match} userId={userId} bestOf={tournament.best_of} />)}</div></section>)}</div>}</section>;
}
...
function MatchPlayer({ name, legs, average, won }: { name: string | null; legs: number | null; average: number | null; won: boolean }) { return <div className={`flex items-center justify-between gap-3 border-b border-white/10 py-2.5 last:border-b-0 ${won ? 'text-emerald-100' : 'text-zinc-300'}`}><span className="min-w-0 truncate font-semibold">{name || 'Wird ermittelt'}{average !== null && <span className="ml-1.5 text-[10px] text-zinc-500">Ø {Number(average).toFixed(1)}</span>}</span>{legs !== null && <strong className={won ? 'text-emerald-200' : 'text-white'}>{legs}</strong>}</div>; }
